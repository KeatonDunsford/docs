...
As we noted, the definition of `type` in the last chapter is a
...
hoon)` to `(map term foot)` when it went into the coil.  A `foot`
...
core.  The product of a dry arm whose hoon is `b` is always
...
`(unit hoon)`; if the unit is full, the name resolves to that
hoon (compiled against the `q` type).  If the unit is empty,
